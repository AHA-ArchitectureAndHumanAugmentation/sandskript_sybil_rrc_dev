...
records what happened and automatically decides which tile to work on
next.

## 1. tile_status.py — what happened, and when
...
so each record is self-contained even if `data/processed/` is later
cleaned out. Saved into `data/executed/`, named so the filename alone
carries everything needed for status checks:
...
| Constant | Purpose |
|---|---|
| `TOTAL_TILES` | Single source of truth for tile count — change it here to scale up or down; `tile_selector.py` reads it from this file |

> `data/robot/` is an **obsolete** earlier version of this same idea
> (small metadata-only records, before the full-copy redesign) — nothing
> reads from it anymore, safe to delete.

## 2. tile_selector.py — what happens next
...
| Limit type | Constants | Behavior |
|---|---|---|
| Rate limit | `MAX_WATER_SPRAYS_PER_WINDOW`, `MAX_SUBSTRATE_SPRAYS_PER_WINDOW`, shared `WINDOW_HOURS` | Temporarily ineligible after too many recent sprays; eligible again once time passes |
| Lifetime cap | `MAX_WATER_SPRAYS_TOTAL`, `MAX_SUBSTRATE_SPRAYS_TOTAL` | `None` by default. If set, **permanently** ineligible once this count is hit |
...
next" can be checked at any later point:
...
- Watering and substrate spraying currently only differ by `spray_type`
  and its separate limits. A physically different toolpath (e.g. a simple
  left-to-right sweep for watering, versus the point-by-point path used
  for substrate) is not yet built.
- Spray protocol itself (how many layers, timing between rounds, the
  actual rules behind "spray x3") is still an open domain question, not a
  code question.
